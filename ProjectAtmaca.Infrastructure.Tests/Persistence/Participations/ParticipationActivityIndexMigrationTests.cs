...
public sealed class ParticipationActivityIndexMigrationTests
{
    [Fact]
    public async Task Migration_Should_CreateCoveringActivityIndex_AndRollbackWithoutRemovingExistingIndexes()
    {
        string database = $"ProjectAtmaca_ActivityIndex_{Guid.NewGuid():N}";
        ...
        try
        {
            ...
            command.CommandText = """
                ...
                WHERE i.object_id = OBJECT_ID(N'dbo.Participations')
                  AND i.name = N'IX_Participations_ActivityReference'
            ...
            keys.Should().Equal(("ActivityReference", false));
            includes.Should().BeEquivalentTo("AtmacaCardId", "Status", "ConditionCode", "JoinedAt", "LeftAt");
            ...
            command.CommandText = """
                ...
                WHERE object_id = OBJECT_ID(N'dbo.Participations')
                  AND name = N'IX_Participations_ActivityReference'
                """;
